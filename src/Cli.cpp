...
char Cli::manage_Input(const vector<char> &options_vector, bool allow_back)
{
    ...
    while (true)
    {
        ...
        {
            if (option == i || (allow_back && (option == 'b' || option == 'B')))
            { // só permitir o 'B' se assim for dito no parametro
                check_to_break = true;
            }
        }
        ...
        if (check_to_break)
            break; // uso o check to break para sair do while loop e não do for :)
...
void Cli::get_Class_Occupation()
{
    ...
    if (std::regex_match(classCode, std::regex("[123](LEIC)([0][123456789]|[1][0-6])")))
    {
        for (auto t : _studentCount)
        {
            ...
            {
                cout << "\nClass " << get<1>(t).substr(0, 7) << "(" << get<1>(t).substr(7, 2) << ")"
                     << " in UC " << get<0>(t) << " has " << get<2>(t) << " students enrolled";
            }
        }
    }
    else
...
bool Cli::permute_Between_Students(const string &studentUp1, const string &studentUp2, const string &ucToSwap)
{
    ...
    {
        if (get<0>(slot) == ucToSwap)
        { // encontrar a uc a mudar no horario do segundo estudante
            bool overlaps = false;
            for (auto slot1 : tmpSchedule1)
            { // ver se ha colisoes no horario do estudante 1
...
bool Cli::permute_One_Student(const string &studentUp1, const string &ucCode, const string &classCodeToChangeTo)
{
    ...
    if (s_search != _setStudent.end())
    {
        for (auto t : stu.get_Schedule())
        {
            ...
            if (get<0>(t).get_Code() == ucCode)
                current_class = get<1>(t).get_ClassCode();
        }

        for (auto c : uc.get_Turmas())
        {
            if (c.get_ClassCode() == classCodeToChangeTo)
                new_date = c.get_ClassDate();
        }

        for (auto d : student_dates)
        {
            if (d.collides(new_date))
            {
                cout << "Permute wasn't possible due to class overposition\n";
                return false;
            }
        }
        ...
        for (auto &tup : _studentCount)
        {
            if (get<0>(tup) == ucCode)
            {
                ...
                if (get<1>(tup).substr(0, 7) == classCodeToChangeTo)
                {
                    ...
                    {
                        cout << "Permute wasn't possible due to an imbalance between classes\n";
                        wait_for_input();
                        return false;
                    }
                }
        ...
        for (auto &tup : _studentCount)
        {
            if (check2)
            {
                if (get<0>(tup) == ucCode && get<1>(tup).substr(0, 7) == current_class)
                    get<2>(tup) -= 1;
            }
        }
        ...
        if (check)
        {
            cout << "UC wasn't found or the Class specified doesn't belong to the UC\n";
            wait_for_input();
            return false;
        }
        return true;
...
void Cli::delist()
{
    ...
    if (s_search != _setStudent.end())
    {
        ...
        if (u_search != _setUc.end())
        {
            ...
            {
                if (get<0>(s).get_Code() == ucCode)
                {
                    uc_check = false;
                    classCode = get<1>(s).get_ClassCode();
                }
            }
